...
| `--api-key` | `TODOLIST_API_KEY` | - | 使用长期 API Key 作为 Bearer 凭据 |
...
除 `health` 和 `auth login` 外，命令需要先登录或提供 API Key。`auth login` 保存的 JWT
按 API 地址保存在用户配置目录，Linux/macOS 下文件权限为 `0600`。自动化或长期运行场景建议
使用桌面端个人中心创建的 API Key，并通过 `TODOLIST_API_KEY` 或 `--api-key` 传入。
...
`TODOLIST_API_KEY` / `--api-key` 使用长期凭据，或通过 `TODOLIST_TOKEN` / `--token`
临时传入访问令牌。
